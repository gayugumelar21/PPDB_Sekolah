    ...
    public function index()
    {
        ...
        if (Auth::check()) {
            ...
            if ($role == 'Admin') {
              ...
              // KERJAAN
              $muridDiproses = Persyaratanppdb::where('status_siswa', 'Diproses')->count();
              $muridDiterima = Persyaratanppdb::where('status_siswa', 'Diterima')->count();
              $muridDitolak = Persyaratanppdb::where('status_siswa', 'Ditolak')->count();

              return view('backend.website.home', compact('guru','murid','alumni','event','acara','book','borrow','member','muridDiproses','muridDiterima','muridDitolak'));
